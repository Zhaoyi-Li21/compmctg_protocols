...
def _get_mcd_idx_1(divergence, examples_1, compounds_1, compounds_2, atoms,
  ...
  for example_idx, example in _shifted_enumerate(examples_1, start_idx):
    ...
    if new_divergence == divergence:
      flag = 1
      temp_idx, temp = example_idx, example
  
  if flag == 0:
    return None, None
  else:
    return temp_idx, temp
...
def _get_mcd_idx_1_min(divergence, examples_1, compounds_1, compounds_2, atoms,
                   get_compounds_fn, get_atoms_fn, start_idx):
  """Return index of example to swap from examples_1 to examples_2."""
  flag = 0
  for example_idx, example in _shifted_enumerate(examples_1, start_idx):
...
def _get_mcd_idx_2_min(divergence, examples_2, compounds_1, compounds_2,
                   get_compounds_fn, start_idx):
  """Return index of example to swap from examples_2 to examples_1."""
  flag = 0
  for example_idx, example in _shifted_enumerate(examples_2, start_idx):
...
def minimize_divergence(examples_1, examples_2, get_compounds_fn, get_atoms_fn,
  ...
  for iteration_num in range(max_iterations):
    ...
    if min_divergence and divergence <= min_divergence:
      # print("Reached divergence target.")
      break
...
def swap_examples(examples_1_,
  ...
  Returns:
    ...
    above.
  """
  examples_1 = examples_1_.copy()
  examples_2 = examples_2_.copy()
